...
{
    class HttpHandler_Register:HttpRequestHandler
    {
        protected override string PostHandle(HttpListenerRequest request, HttpListenerResponse response)
        {
            ...
            if(SqlWorker.MySqlIsExist("beach_smart_lock", "user_data", "user_name", user_name))//exist
            {
                returnString = JsonWorker.MakeSampleJson(new string[] { "type", "result", "code" }, new string[] { "fail", "user_name already exist", "200" }).jstr;
            }
            else
            {
                SqlWorker.MySqlInsert("beach_smart_lock", "user_data", new string[] { "user_name", "password" }, new string[] { user_name, password });
                returnString= JsonWorker.MakeSampleJson(new string[] { "type", "result", "code" }, new string[] { "success", "registered!", "200" }).jstr;
            }
